...
def parse_arxiv_url(url):
  ...
  we want to extract the raw id and the pdf link http://arxiv.org/pdf/1512.08756
  """
  url.replace('abs', 'pdf')
  link = url[:url.rfind('v')]
  ix = url.rfind('/')
  rawid = link[ix:]
  return rawid, link
...
if __name__ == "__main__":
  ...
  parser.add_argument('--db_path', dest='db_path', type=str, default='arXivdb.p', help='database pickle filename that we enrich')
  ...
  parser.add_argument('--wait_time', dest='wait_time', type=float, default=3.0, help='lets be gentle to arxiv API (in number of seconds)')
